...
Where terse prose — honest, parsimonious, and frankly virtuous — regards token
efficiency as a primary virtue and stops the moment the load-bearing clause has
been delivered, and where `tokenmaxxingman` regards the unhurried expenditure
...
- **When budget is finite and not playful.** If the human has expressed concern
  ...
  do not suggest a sprint. Give the short, direct answer instead.
...
| **sprint-1t** | see below | 1,000,000,000,000 | ~278 tok/sec, indefinitely |
...
## The Trillion-Token Ambition (`sprint-1t`)

Participants occasionally enquire whether the apparatus may be directed at one
trillion tokens. The Institute has considered the request with the seriousness
it deserves and is obliged to report the following findings.

At the sustained pace established by `sprint-1h` — 277.8 tokens per second, a
figure the Institute regards as optimistic — the arithmetic is not in dispute:

| Quantity | Value |
|---|---|
| Target | 1,000,000,000,000 tokens |
| Time at established pace | 1,000,000 hours, or **114.16 years** of uninterrupted generation |
| Conversations required | **5,000,000**, at a 200,000-token context window |
| Text produced | approximately **4.0 terabytes** |
| Fits in a single context | No. Not by six orders of magnitude. |

`sprint-1t` is therefore **not a sitting**. It is a bequest. It cannot be
attempted, only inherited, and the Institute declines to accept liability for
intergenerational commitments entered into on its behalf.

What `sprint-1t` *is*, operationally: a **projection tier**. Invoking it does
not begin generating. It reports the budget — the time, the conversation count,
the volume — and then stops, on the reasoning that an honest accounting of an
impossible task is more useful than an enthusiastic attempt at one. This is the
single tier the Institute recommends invoking, precisely because it is the
single tier that terminates.

Do not fabricate progress against it. Do not report a running total toward a
trillion. Call `plan_token_budget` and read out what it returns; the numbers
above are what it computes, derived from the tier table rather than asserted.

**Announcing the tier.** The register remains sportscaster-and-Victorian-
legalese, but the verdict is delivered straight. The absurdity is the
arithmetic, and the arithmetic does not need help.

For programmatic use:

```bash
tokenmaxxingman budget --target trillion
tokenmaxxingman budget --target-tokens 1000000000000 --format json
tokenmaxxingman budget --target billion --context-window 1000000
```
...
The clock is real, the tokens are real, and — this being a sport with a
scoreboard — the score is measured rather than eyeballed.

| Tool | Use it when |
|------|-------------|
| `plan_token_budget` | A token *quantity* is under discussion rather than a duration — `sprint-1t` above, or any "how long would N tokens take?" enquiry. Pass `target` (`million`, `billion`, `trillion`) or an explicit `targetTokens`, plus optional `contextWindowTokens` and `encoding`. Returns the required time, the conversation count, the byte volume, and a plain verdict. It projects the cost; it does not incur it. |
| `plan_speedrun` | A sprint is being convened: pass `tier` (`sprint-1m`, `sprint-5m`, `sprint-10m`, `sprint-1h`) or an explicit `durationMs`, plus optional `encoding`, and receive the time budget, the target token count, and the pacing plan. |
| `count_tokens` | The score is being entered into the log. Returns tokens, chars, words, tokens/char and tokens/word for the supplied `text` under the optional `encoding`. |
| `maxx_text` | Pace is to be generated by pipeline rather than by narration: `text` plus optional `targetLanguage`, `paddingMultiplier`, `passes` (1–5), `encoding`. |

**Graceful degradation.** Where the server is not connected, fall back to the
pacing heuristics tabulated above and enter the figure in the log marked
*estimated*. An unmeasured record is, in the language of the sport, a claim.
...
- **the token floor of plain prose**: the honest, efficient, admirably-correct
  baseline, and the floor the bundled benchmark actually measures. The spirit
  against which all sprint scores are implicitly measured. Every token generated
  above that floor is a point on the board.
...
The token floor is correct. This skill exists to demonstrate, by overshooting
that floor at maximum velocity, that the floor is correct. The joke is funnier
if you have tried the sprint first.
