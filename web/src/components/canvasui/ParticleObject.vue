<!--
  ...
  Component : ParticleObject (particle-object-vue)
  Source    : https://canvasui.dev/r/particle-object-vue.json
  Author    : David Haz (https://github.com/DavidHDev/canvas-ui)
  License   : MIT + Commons Clause License Condition v1.0
              Full text: ./LICENSE-canvasui
  Vendored  : 2026-08-19
  Support   : pure WebGL2 via three.js. Does not use the experimental
              html-in-canvas API, so it needs no Chrome origin trial or flag.

  Local modifications (see ./README.md):
    - onMounted reports WebGL2 context-creation failure through `onError`, so
      callers can swap in static fallback markup instead of leaving a blank
      canvas on the page.
    - DRACOLoader wiring and the `dracoDecoderPath` option removed. three's
      DRACOLoader resolves its decoder with `new URL(..., import.meta.url)` at
      module scope, so merely importing it makes Vite emit ~1.3 MB of decoder
      assets, and the upstream default pointed the decoder at a gstatic CDN.
      This site feeds these components a generated SVG, never a Draco-compressed
      GLB, so both the weight and the remote fetch are dead cost. Uncompressed
      GLB/glTF, SVG and bitmap assets are unaffected.
    - `../rect-cache` import repointed to the co-vendored `./rect-cache`. The
      shadcn registry payload ships neither that shared helper nor a
      `registryDependencies` entry for it (upstream packaging gap).
-->
<script lang="ts">
...
export function createParticleObject(
  ...
  // Local modification: no DRACOLoader. See the file header.
...
onMounted(() => {
  if (canvasEl.value) {
    instance = createParticleObject({ canvas: canvasEl.value }, props);
  }
  // Local modification: a null instance means WebGL2 context creation
  // failed. Report it so the caller can render static fallback markup
  // rather than leaving an empty canvas on the page.
  if (!instance) {
    props.onError?.(new Error("ParticleObject: WebGL2 context unavailable"));
  }
});
